...
async def test_bond_over_one_month_is_red(corpus_session):
    findings = await run(corpus_session, lease(bond_amount=Decimal(2500)))
...
and skips with "Statutory basis not in force". The `2166.67` figure in
the red-bond test is 500 weekly x 52 / 12 = 2166.666... -> 2166.67; the
default 2000-monthly lease's monthly cap is 2000 x 12 / 52 x 52 / 12 =
2000.00... wait - the default lease is ALREADY monthly, so weekly =
2000 x 12 / 52 = 461.54, monthly cap = 461.54 x 52 / 12 = 2000.01
(rounding round-trip). The first red test uses the DEFAULT lease with
bond 2500 > 2000.01 -> red, and its summary asserts "2166.67"? No -
FIX: that assertion belongs to a weekly-500 lease. In the first test
change the lease to `lease(rent_amount=Decimal(500), rent_frequency="weekly",
bond_amount=Decimal(2500))` so the cap is exactly 2166.67 and the
assertion holds. Keep the boundary test's 2166.67 green as written.
