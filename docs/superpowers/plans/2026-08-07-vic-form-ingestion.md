...
**Goal:** All 24 prescribed forms' numbered terms in VIC Regs 2021 Schedule 1 (roughly 236, including Form 1's 32 and Form 2's 40 rental-agreement terms) land in the corpus point-in-time under `S1-F{k}-T{m}` keys.

**Architecture:** `parse_docx` gains a form sweep active inside schedule regions: "New Form Heading" paragraphs open form scopes and titles, raw-text `^N.\t` paragraphs start terms (the tab must be matched before `_clean` collapses it), Side Notes are skipped within forms only; the VIC Regulations instrument is wiped and rebuilt from the DOCX cache; the CI dump is refreshed; production follows over the tunnel.
...
roughly 236; `S1-F1-T6` resolves today (its heading is the Rent term);
